 ...
 *
 * This page is mostly a form, and a browser proved it renders — but a form has
 * one interesting question and it is not layout: what it does with an answer
 * from the host. The reports this plugin surfaces are the whole reason the
 * plugin is debuggable at all, and the failure that motivated them is
 * indistinguishable by eye: a run that found no Chinese and a run that failed
 * on every single file both used to render as "nothing to translate". That is
 * a pure function of a report body, so it is tested here rather than by
 * clicking.
 *
 * A hook-faithful React stub, a `fetch` that answers by route, and the bundle
 * evaluated the way the loader evaluates it —
 ...
 * run. `dom` supplies a DOM stub for the nav-icon repaint; the default is
 * headless, which is the case the CSS injection and the observer both guard
 * against.
 *
 * @module @creait/dsh-to-english/test/client-harness
 */
...
/**
 * One nav cell: a label and an svg.
 *
 * This plugin's glyph is six paths against the gear's two, so it replaces the
 * svg's whole child list rather than retouching path by path — the stub
 * therefore records `innerHTML` and `viewBox` rather than per-path attributes.
 */
export function makeNavCell(label) {
	const svg = { dataset: {}, innerHTML: '<path d="gear-ring"/><path d="gear-dot"/>', attrs: {}, setAttribute(k, v) { this.attrs[k] = v; } };
 ...
 *
 * @param options - `config`, `catalog` and `status` are the three GET payloads,
 *   each a value or a thunk (a thunk is how a test makes the status re-read
 *   after a manual run differ from the first one); `post` answers a settings
 *   write and `translate` answers a manual run, both defaulting to success.
 *   Any of them may be `'network'` for a rejected fetch. `primitives: true`
 *   serves the seed-module stub; `dom` supplies a DOM (from `makeDom`) instead
 *   of running headless; `locale: 'throws'` makes `locale.register` throw,
 *   which is what a client reload does.
...
export function loadClient(options = {}) {
	...
	const defaultView = { writable: true, value: {} };

	const answerFor = (request) => {
		if (request.method === 'POST' && request.url === TRANSLATE_ROUTE) return options.translate ?? { body: { status: 'done', translated: [], errors: [] } };
		if (request.method === 'POST') return options.post ?? { body: { ...defaultView, value: request.body } };
		...
		if (request.url === STATUS_ROUTE) return typeof options.status === 'function' ? options.status() : (options.status ?? { body: null });
	...
	return { id: registered.id, inject: exported.inject, Component: component, descriptor, locales, effects, mount, requests, warnings, primitives, React };
}
...
export function buttonsOf(instance) {
	return walk(instance.output).filter((node) => typeof node.type === 'function' && node.props.onClick !== undefined && node.props.options === undefined);
}

/** The `role="switch"` toggles, keyed by their `aria-label`. */
export function togglesOf(instance) {
	const found = {};
	for (const node of walk(instance.output)) {
		if (node.props?.role === 'switch') found[node.props['aria-label']] = node;
	}
	return found;
}

/** The package-name box, whichever of the two shapes it took. */
export function packageInput(instance) {
	return walk(instance.output).find((node) => node.props?.['aria-label'] === 'packageNamePlaceholder');
}

/** The prompt editor. */
export function promptBox(instance) {
	return walk(instance.output).find((node) => node.props?.className === 'te-textarea');
}
